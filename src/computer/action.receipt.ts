...
export function matchHitElement(
  ...
  const ranked = chain.map(recipient => {
    ...
    const nativeLabels = [recipient.identifier, recipient.title, recipient.description].map(normalize).filter(Boolean);
    if (labels.length && nativeLabels.length) {
      if (nativeLabels.some(actual => labels.some(wanted => actual === wanted || actual.includes(wanted) || wanted.includes(actual)))) {
