...
export class TestHealer {
  ...
  async heal(opts?: { testCommand?: string; maxRounds?: number }): Promise<HealReport> {
    ...
    const first = await this.runTests(this.projectRoot, testCommand);
    if (first.passed) {
    ...
    if (!this.isGitRepo()) {
      this.log('error', 'Self-healing needs a git repository (the fix runs in a worktree).');
      return report;
    }
