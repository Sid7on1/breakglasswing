...
describe('WorktreeManager — lifecycle against a real temp git repo', () => {
  ...
  // Regression: a real /beast run fired 4 parallel `git worktree add`s (SwarmOrchestrator wave) and
  // they raced on the shared .git/worktrees/ registry → "failed to read .git/worktrees/<id>/commondir:
  // Undefined error: 0". The manager now serializes registry-mutating ops; concurrent creates must all
  // succeed. This is exactly the swarm/speculate parallel-wave pattern.
  it('handles many CONCURRENT worktree creates without corrupting the registry', async () => {
    const branches = Array.from({ length: 6 }, (_, i) => `swarm/race/task-${i + 1}`);
    const results = await Promise.all(branches.map(b => mgr.createWorktree(b, 'HEAD')));
    for (const { worktreePath } of results) expect(fs.existsSync(worktreePath)).toBe(true);
