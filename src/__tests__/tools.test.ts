...
describe('TodoWriteTool', () => {
  ...
  it('rejects a claimed loaded search after only opening the browser, then accepts post-Return proof', async () => {
    clearActiveTodos();
    const content = "Open Safari and search the web for 'easy vegetarian dinner ideas'. Verify the results page is visibly loaded.";
    await tool.execute({ todos: [{ content, status: 'in_progress' }] });
    const result = (value: Record<string, unknown>) => ({
      role: 'tool' as const, tool_call_id: String(value.action),
      content: JSON.stringify({ ok: true, driver: 'bimax-computer-use 0.12.3', app: 'Safari', ...value }),
    });

    const rejected = await tool.execute({ todos: [{ content, status: 'completed' }] }, {
      sessionMessages: [result({ action: 'open', screenshot: '/tmp/start.png' })],
    });
    expect(rejected).toMatch(/Todo completion rejected.*proves the content was committed/i);
    expect(getActiveTodos()[0].status).toBe('in_progress');

    const accepted = await tool.execute({ todos: [{ content, status: 'completed' }] }, {
      sessionMessages: [
        result({ action: 'type', screenshot: '/tmp/query.png' }),
        result({ action: 'key', summary: 'pressed return in Safari — screen changed', screenshot: '/tmp/results.png' }),
      ],
    });
    expect(accepted).toContain('1/1 done');
    expect(getActiveTodos()[0].status).toBe('completed');
    clearActiveTodos();
  });

  it('applies the completion gate to any app, not a hardcoded set', async () => {
    // The gate once named Safari/WhatsApp/Calculator/TextEdit/Finder explicitly, so a task in any
    // other app was waved through. Apps come from the recorded evidence, so an app nobody tested
    // with gets identical treatment.
    clearActiveTodos();
    const content = 'Post the release note in Obsidian and verify it is visible in the note body.';
    await tool.execute({ todos: [{ content, status: 'in_progress' }] });
    const result = (value: Record<string, unknown>) => ({
      role: 'tool' as const, tool_call_id: String(value.action),
      content: JSON.stringify({ ok: true, driver: 'bimax-computer-use 0.12.3', app: 'Obsidian', ...value }),
    });

    const rejected = await tool.execute({ todos: [{ content, status: 'completed' }] }, {
      sessionMessages: [result({ action: 'type', screenshot: '/tmp/typed.png' })],
    });
    expect(rejected).toMatch(/Todo completion rejected/i);
    expect(getActiveTodos()[0].status).toBe('in_progress');

    const accepted = await tool.execute({ todos: [{ content, status: 'completed' }] }, {
      sessionMessages: [
        result({ action: 'type', screenshot: '/tmp/typed.png' }),
        result({ action: 'key', summary: 'pressed return in Obsidian', screenshot: '/tmp/posted.png' }),
      ],
    });
    expect(accepted).toContain('1/1 done');
    clearActiveTodos();
  ...
  it('does not gate an ordinary code checklist that happens to share a session with computer use', async () => {
    // "save" and "result" are everywhere in engineering checklists. Only a claim about a GUI surface
    // may be held to GUI evidence; a code task must never be blocked because a desktop action was
    // recorded earlier in the same session.
    clearActiveTodos();
    const content = 'Refactor the config loader and save the parsed result to disk';
    await tool.execute({ todos: [{ content, status: 'in_progress' }] });
    const accepted = await tool.execute({ todos: [{ content, status: 'completed' }] }, {
      sessionMessages: [{
        role: 'tool' as const, tool_call_id: 'open',
        content: JSON.stringify({
          ok: true, driver: 'bimax-computer-use 0.12.3', action: 'observe',
          app: 'Notes', screenshot: '/tmp/notes.png',
        }),
      }],
    });
    expect(accepted).toContain('1/1 done');
