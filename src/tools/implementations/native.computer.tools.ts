...
import { NativeServiceOperationClient } from '../../computer/native.bridge.transport';
import { resolveBrowserWorkspacePath } from '../../browser/browser.runtime';
import { resolveConvergedComputerRoute } from '../../computer/browser.convergence.route';
import type { NativeRolloutController } from '../../computer/native.rollout';
import { workspaceWriteBlock } from '../path.util';
import { buildNativeOperationToolContracts } from '../../computer/native.operation.contract';
import {
...
} from '../../computer/native.tool.coordinator';
import {
  assessNativeCutover,
...
import { buildTool, type BuiltTool } from '../tool.factory';
...
function taintObservation(kind: string): void {
  try { getTaintTracker().mark('web', `native computer ${kind}`); } catch { /* best effort */ }
}
...
async function approveNativeAction(
  ...
  const app = await coordinator.appIdentity(taskSessionId, prepared.target.pid);
    ...
    execute: async (args: Record<string, unknown>, context?: unknown): Promise<string> => {
      ...
      try {
        ...
        case 'BimaxActionTool': {
          ...
          const approval = await approveNativeAction(governor, coordinator, session, input, prepared);
          return json({
