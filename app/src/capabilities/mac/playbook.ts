...
export function shouldUseFlashComputerPlaybook(model: string): boolean {
  ...
  // The app's live 2026-08-13 Calculator probe showed this controller losing the task inside the
  // full manual and narrating prospective calls. It remains tool-capable; use the compact strict
  // loop that keeps one action and its fresh returned frame in focus.
...
export function buildComputerUseModelPrompt(
  ...
  const playbook = shouldUseFlashComputerPlaybook(options.model || '')
    ? COMPUTER_USE_FLASH_PLAYBOOK
    : computerUsePlaybookFor(prompt);
  const toolName = options.toolName || 'mac_control';
  const evidence = options.native
    ...
    : `[Fresh computer-use constraint: Complete this turn only from ${toolName} results produced after this request. Prior shell, browser, assistant, memory, and tool values are not evidence. Continue until the newest result proves every requested value; otherwise report the concrete runtime blocker.]`;
  return `${prompt}\n\n${playbook}\n\n${evidence}`;
}
