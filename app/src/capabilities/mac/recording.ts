...
export class RecordingController {
  ...
  /** Legacy, un-namespaced location. Used only when the caller supplies no `storageRoot`. */
  private legacyRoot(cwd: string): string {
    return path.join(cwd, '.bimax', 'computer', 'recordings');
  }

  private defaultDir(cwd: string, storageRoot?: string): string {
    const root = storageRoot || this.legacyRoot(cwd);
  ...
  async start(opts: {
    ...
    outputDir?: string;
    /**
     * Per-session recordings root. The runtime namespaces durable state per Bimax task, so
     * recordings must be swept and written under that session's directory rather than the single
     * global one — otherwise one task's sweep evicts another task's runs.
     */
    storageRoot?: string;
    recordVideo: boolean;
    ...
    sweepRecordings(opts.storageRoot || this.legacyRoot(opts.cwd), { keepRuns: 5 });
    const dir = path.resolve(opts.outputDir || this.defaultDir(opts.cwd, opts.storageRoot));
