...
import type { BimaxComputerRuntime, DesktopResult } from '../../src/capabilities/mac/desktop.runtime';
import type { TaskRun } from '../../../src/telemetry/task.metrics';
import { buildComputerUseModelPrompt } from '../../src/capabilities/mac/playbook';
...
function buildFixture(): string {
  const out = execFileSync('bash', [path.join(REPO_ROOT, 'app/scripts/computer-use/build-bimax-cu-fixture.sh'), FIXTURE_INSTALL_PATH], {
...
async function main(): Promise<void> {
  ...
  if (process.argv.includes('--self-test')) {
    buildFixture();
    const { BimaxComputerRuntime: R } = await import('../../src/capabilities/mac/desktop.runtime');
    const runtime = new R();
  ...
  const [
    { AgentLoop }, { ToolRegistry }, { McpManager },
    ...
    import('../../src/capabilities/mac/desktop.runtime'),
  ...
  for (const task of selected) {
    for (let attempt = 1; attempt <= repeats; attempt++) {
      ...
      try {
        ...
        const providerBinary = path.join(REPO_ROOT, 'app/native-service/bimax-mac-capability');
        const providerEnv: Record<string, string> = {
          BIMAX_CWD: cwd,
          BIMAX_HOST_ARCH: process.arch === 'arm64' ? 'arm64' : 'x64',
        ...
        for (const [key, relative] of Object.entries({
          BIMAX_CU_SERVICE_BINARY: 'app/native-service/BimaxCuService.xpc/Contents/MacOS/bimax-cu-service',
          BIMAX_CU_BRIDGE_BINARY: 'app/native-service/bimax-cu-bridge',
          BIMAX_DESKTOP_HELPER: 'app/native-service/bimax-desktop-helper',
        })) providerEnv[key] = path.join(REPO_ROOT, relative);
        const mcp = new McpManager();
        const connection = await mcp.connectSpec({
          name: 'bimax-mac', command: providerBinary, args: [], env: providerEnv,
          type: 'stdio', eager: true, forceScrubEnv: true,
        }, registry, governor);
        if (!connection) throw new Error(`Mac capability provider did not start at ${providerBinary}`);
        const macToolName = connection.toolNames.find((name: string) => name.endsWith('__mac_control'));
        if (!macToolName) throw new Error('Mac capability provider did not expose mac_control');
        ...
        const captureAction = (entry: any) => {
          if (entry?.toolName !== macToolName || actionTrace.length >= 30) return;
        ...
        try {
          ...
          for await (const chunk of loop.execute(
            [{ role: 'user', content: buildComputerUseModelPrompt(task.prompt, { model: config.model, toolName: macToolName }) }],
            persona.getSystemPrompt({ toolNames: [macToolName] }),
            {
              maxIterations,
              contextMode: 'smart',
              metricsLabel: task.taskClass,
              toolNames: [macToolName],
              skipRepoMap: true,
              requireTool: macToolName,
            },
          ...
          await mcp.disconnect('bimax-mac', registry);
        }
...
/**
 * The executor that produced a measurement. A run whose backend was never recorded is NOT folded
 * into a known one — it becomes its own `unattributed` group, because a number whose executor is
 * unknown cannot be attributed to either the native or the compatibility route.
 */
export const UNATTRIBUTED_BACKEND = 'unattributed';

export function backendOf(m: Measurement): string {
  const raw = m.backend?.trim();
  return raw ? raw : UNATTRIBUTED_BACKEND;
}
...
export function buildReport(
  ...
  // Rows are keyed by BACKEND FIRST, then task class. `07_MIGRATION_ROADMAP.md` Phase 2: "Run
  // native and compatibility baselines separately; never combine their numbers", and the Desktop
  // computer-use gate says native evaluation "is reported separately from the narrow exact-state
  // benchmark". Merging them would produce one pass rate describing no single system — the same
  // error the model-change disqualifier below already guards against.
  const backends = [...new Set(valid.map(backendOf))].sort();
  const rows = backends.flatMap(backend => {
    const forBackend = valid.filter(m => backendOf(m) === backend);
    const classes = [...new Set(forBackend.map(m => m.taskClass))];
    return classes.map(taskClass => {
      const group = forBackend.filter(m => m.taskClass === taskClass);
  ...
  const header = '| backend | task class | runs | completed | median turns | median tool calls | median prompt tokens | median wall clock |';
  const divider = '|---|---|---|---|---|---|---|---|';
  const body = rows.map(r => `| ${r.backend} | ${r.taskClass} | ${r.runs} | ${r.completed} | ${r.medianTurns} | `
  ...
  const unattributed = valid.filter(m => backendOf(m) === UNATTRIBUTED_BACKEND);
  const disqualifiers = [
    models.length > 1
      ? `the work model changed mid-suite (${models.join(' → ')}), so these rows are not all the same system`
      : '',
    // Same reasoning as the model check, one level down: two executors are two systems. The rows
    // above are already separated, so this does not hide anything — it refuses to call a suite
    // that measured two different routes a single frozen baseline for either of them.
    backends.length > 1
      ? `this suite mixed ${backends.length} executors (${backends.join(', ')}); native and compatibility numbers are reported separately and are never a single baseline`
      : '',
    // A run whose executor was never recorded cannot be attributed to either route. Counting it
    // silently is how an unattributed measurement ends up inside a frozen denominator.
    unattributed.length > 0
      ? `${unattributed.length} valid run(s) recorded no backend (${[...new Set(unattributed.map(m => m.task))].join(', ')}), so their executor is unknown`
      : '',
  ...
  const verdict = disqualifiers.length
    ...
    : `\n\n> Frozen baseline for the **${backends[0]}** executor: one model throughout, one attributed `
      + 'executor, no discarded runs, at least 3 valid runs per class. It is a baseline for this '
      + 'executor only and may not be quoted for any other.';
